/*
  ...
  - React 컴포넌트는 한 번 정의하면 여러 곳에서 재사용이 가능하다. 
    효율적인 컴포넌트 구조화를 위해 다음 원칙을 지키면 좋다.

  3-1) 컴포넌트 분리
  - 하나의 컴포넌트에 많은 기능이 담겨 있으면 유지보수가 어렵다. 
  - 기능별로 컴포넌트를 분리하여 관리하면 재사용성이 높아지고, 코드 가독성도 좋아진다.
...
/*
  3-2) 컴포넌트 조합
    ...
    - 컴포넌트는 다른 컴포넌트 안에 포함될 수 있다. 이를 컴포지션(Composition)이라고 하며, 
      여러 개의 작은 컴포넌트를 조합해 하나의 큰 컴포넌트를 구성할 수 있다.
*/
...
/*
  ...
  - React에서 데이터는 상위 컴포넌트에서 하위 컴포넌트로 props를 통해 전달되며 이를 통해 데이터를 공유하고 관리할 수 있다.
