...
function ControlledComponent() {
  ...
  const inputRef1 = useRef(null); // ref를 만들어서 입력 요소에 연결
  ...
  const handleClick = () => { 
    console.log(inputRef1);         // ref 객체
    console.log(inputRef1.current); // input 요소에 대한 참조
    console.log(inputRef1.current.value);  // input 요소의 값
    console.log("비제어 입력값1: " + inputRef1.current.value); 
    let price = inputRef1.current.value;
    price = Number(price);
    let totalPrice = price + price * 0.1;
    inputRef2.current.value = totalPrice;
  };
  ...
  return (
    <>
      ...
      <div>
        <p>금액 : <input
          type="text"
          ref={inputRef1} // ref를 사용하여 DOM 요소에 직접 접근
          placeholder="상태관리 x"
        />
        </p>
        <p>
          부가세 포함 금액 : <input
            type="text"
            ref={inputRef2} // ref를 사용하여 DOM 요소에 직접 접근
            placeholder="상태관리 x"
            readOnly
        />
        </p>
